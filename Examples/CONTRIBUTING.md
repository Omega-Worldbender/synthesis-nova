...
/examples/                    the kit in code: one-page prompt, customer-service prompt,
                              Python (Anthropic · OpenAI-compatible · CS bot) and TypeScript
                              clients that load FULL+ v2.0 and check the first reply
/legacy/                      prior versions, prior papers, prior copyright descriptions,
                              the v3.8 examples — the record. Read-only by convention.
```
...
- **Integrations** in `/examples/`: a client for another SDK or language, as long as it loads the FULL+ file from the repo root (or `quick_start_v9.txt` as fallback), appends the user chart rather than editing Part A, keeps context tags out of stored history, and exposes the first-reply check. Provider model names go in an environment variable with a documented default; they go stale faster than the repo.
